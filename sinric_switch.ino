...
#define APP_SECRET        "APP_SECRETE"     // Should look like "5f36xxxx-x3x7-4x3x-xexe-e86724a9xxxx-4c4axxxx-3x3x-x5xe-x9x3-333d65xxxxxx"
#define SWITCH_ID         "SWITCH_ID"      // Should look like "5dc1564130xxxxxxxxxxxxxx"
#define BAUD_RATE         115200                // Change baudrate to your need

#define BUTTON_PIN        4   // GPIO for BUTTON (inverted: LOW = pressed, HIGH = released)
...
bool onPowerState(const String &deviceId, bool &state)
{
  ...
  myPowerState = state;
  digitalWrite(LED_PIN, myPowerState ? HIGH : LOW); // test 23 Oct 2022 - maintain this (HIGH:LOW)
...
void handleButtonPress()
{
  ...
  if (digitalRead(BUTTON_PIN) == LOW && actualMillis - lastBtnPress > 1000)
  {
    ...
    if (myPowerState)
    {
      ...
      myPowerState = false; // test on 23/10/2022 final (false)
    }
    else
    {
      myPowerState = true; // test on 23/10/2022 final (true)
    }
    // test on 23/10/2022 final (HIGH:LOW)
